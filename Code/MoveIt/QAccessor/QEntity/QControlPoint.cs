...
namespace MoveIt.QAccessor
{
    internal partial struct QEntity
    {
        ...
        private readonly bool ControlPoint_SetUpdated()
        {
        ...
        private readonly bool ControlPoint_MoveBy(State state, float3 newPosition, float3 delta)
        {
            return ControlPoint_MoveTo(state, newPosition, delta);
        }

        private readonly bool ControlPoint_MoveTo(State state, float3 newPosition, float3 delta)
        {
            RefRW<MIT_ControlPoint> cpRef = m_Lookup.MIT_ControlPoint.GetRefRW(m_Entity);
            Entity parent = cpRef.ValueRO.m_Parent;
        ...
        private readonly bool ControlPoint_RotateBy(State state, float delta, ref Matrix4x4 matrix, float3 origin)
        {
        ...
        private readonly bool ControlPoint_RotateTo(State state, quaternion newRotation, ref Matrix4x4 matrix, float3 origin)
        {
            RefRW<MIT_ControlPoint> cpRef = m_Lookup.MIT_ControlPoint.GetRefRW(m_Entity);
