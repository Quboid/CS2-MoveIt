...
namespace MoveIt.Tool
{
    public partial class MIT : ObjectToolBaseSystem
    {
        ...
        internal bool IsManipulator(Entity e)
        {
            if (!(
                EntityManager.HasComponent<Game.Net.Edge>(e)
                )) return false;
        ...
        internal bool IsManipulated(Entity e)
        {
            if (!(
                EntityManager.HasComponent<Components.MIT_ControlPoint>(e)
                )) return false;
        ...
        internal void UpdateMarqueeList(Input.Marquee marquee)
        {
            ...
            using Searcher.Marquee searcher = new(Searcher.Filters.All & ~Searcher.Filters.Segments, f);
            ...
            for (int i = 0; i < searcher.m_Results.Length; i++)
            {
        ...
        private void GenerateUpdateGrid(Input.Marquee marquee, Bounds3 bounds, out NativeArray<Bounds3> grid, out NativeArray<SelectionGridModes> mode)
        {
            ...
            //};
            ////if (!m_LastMarqueeBounds.Equals(new(float.MaxValue, float.MaxValue))) AddDebugBounds(m_LastMarqueeBounds, Overlay.Colors.Unselect);
